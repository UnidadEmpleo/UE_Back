...
param(
    ...
    [string]$ApiDomainName = "apiclinica.cusaem.gob.mx",
    ...
    [string]$ClinicaDatabaseName = "Clinica",
    [string]$SqlUser = "",
    [string]$SqlPassword = "",
...
function Install-Certificate {
    ...
    if ($ExistingCertThumbprint) {
        Write-Log "Buscando certificado existente por Thumbprint: $ExistingCertThumbprint" "INFO"
        $cert = Get-ChildItem -Path Cert:\LocalMachine\My | Where-Object { $_.Thumbprint -eq $ExistingCertThumbprint }
        if ($cert) {
            $script:CertThumbprint = $cert.Thumbprint
            Write-Log "Certificado encontrado y seleccionado: $($script:CertThumbprint)" "SUCCESS"
        } else {
            Write-Log "No se encontró un certificado con el Thumbprint especificado" "ERROR"
            throw "Certificado no encontrado en el almacén local."
        }
...
function Build-Frontend {
    ...
    $frontendPath = Join-Path $RepoPath "SIPRED-WEB"
    
    if (-not (Test-Path $frontendPath)) {
        throw "No se encontro la carpeta SIPRED-WEB en $RepoPath"
    }
...
function New-Database {
    ...
    Write-Log "Creando bases de datos..." "INFO"
    ...
    try {
        ...
        # Crear base de datos Clinica
        Write-Log "Creando base de datos: $ClinicaDatabaseName" "INFO"
        $createClinicaDbScript = "IF NOT EXISTS (SELECT name FROM sys.databases WHERE name = '$ClinicaDatabaseName') BEGIN CREATE DATABASE [$ClinicaDatabaseName]; END"
        $cmdClinica = New-Object System.Data.SqlClient.SqlCommand($createClinicaDbScript, $conn)
        $cmdClinica.ExecuteNonQuery() | Out-Null
        Write-Log "Base de datos $ClinicaDatabaseName configurada" "SUCCESS"
        
        $conn.Close()
        
        Write-Log "Todas las bases de datos fueron configuradas correctamente" "SUCCESS"
...
function Deploy-ToIIS {
    ...
    # ============================================
    # DETENER IIS Y APPLICATION POOLS
    # ============================================
    Write-Log "Deteniendo IIS y Application Pools..." "INFO"
    
    $appcmd = "$env:SystemRoot\System32\inetsrv\appcmd.exe"
    
    # Detener sitios si existen
    @("BaseWebApp-API", "BaseWebApp-Frontend") | ForEach-Object {
        $siteExists = & $appcmd list site $_ 2>$null
        if ($siteExists) {
            Write-Log "Deteniendo sitio: $_" "INFO"
            & $appcmd stop site /site.name:$_ 2>$null
        }
    }
    
    # Detener Application Pools si existen
    @("BaseWebApp-API-Pool", "BaseWebApp-Frontend-Pool") | ForEach-Object {
        $poolExists = & $appcmd list apppool $_ 2>$null
        if ($poolExists) {
            Write-Log "Deteniendo Application Pool: $_" "INFO"
            & $appcmd stop apppool /apppool.name:$_ 2>$null
        }
    }
    
    # Detener IIS completamente
    Write-Log "Deteniendo servicio IIS..." "INFO"
    net stop was /y 2>$null | Out-Null
    iisreset /stop 2>$null | Out-Null
    
    # Esperar a que se liberen los archivos
    Write-Log "Esperando a que se liberen los archivos..." "INFO"
    Start-Sleep -Seconds 5
    
    # Forzar cierre de procesos w3wp si aún existen
    $w3wpProcesses = Get-Process w3wp -ErrorAction SilentlyContinue
    if ($w3wpProcesses) {
        Write-Log "Terminando procesos w3wp bloqueantes..." "WARNING"
        $w3wpProcesses | Stop-Process -Force -ErrorAction SilentlyContinue
    ...
    # CREAR DIRECTORIOS
    # ============================================
    Write-Log "Creando directorios de despliegue..." "INFO"
    ...
    # ============================================
    Write-Log "Limpiando archivos anteriores..." "INFO"
    
    # Intentar limpiar API (con reintentos)
    $maxRetries = 3
    $retryCount = 0
    $apiCleaned = $false
    
    while (-not $apiCleaned -and $retryCount -lt $maxRetries) {
        $retryCount++
        
        try {
            if (Test-Path "$apiDest\*") {
                Write-Log "Intento ${retryCount} de ${maxRetries}: Limpiando carpeta API..." "INFO"
                Remove-Item "$apiDest\*" -Recurse -Force -ErrorAction Stop
                $apiCleaned = $true
                Write-Log "Carpeta API limpiada correctamente" "SUCCESS"
            } else {
                $apiCleaned = $true
            }
        } catch {
            if ($retryCount -lt $maxRetries) {
                Write-Log "No se pudieron eliminar algunos archivos. Reintentando en 3 segundos..." "WARNING"
                
                # Intentar matar cualquier proceso que tenga archivos abiertos
                Get-Process | Where-Object { $_.Path -like "$apiDest*" } | Stop-Process -Force -ErrorAction SilentlyContinue
                
                Start-Sleep -Seconds 3
            } else {
                Write-Log "No se pudieron eliminar todos los archivos despues de ${maxRetries} intentos" "WARNING"
                Write-Log "Algunos archivos pueden no actualizarse" "WARNING"
            }
        }
    }
    
    # Limpiar Frontend (usualmente no tiene problemas)
    if (Test-Path "$frontendDest\*") {
        Write-Log "Limpiando carpeta Frontend..." "INFO"
    ...
    # Verificar que FrontendDistPath fue establecido
    if (-not $script:FrontendDistPath -or -not (Test-Path $script:FrontendDistPath)) {
        throw "No se encontro la carpeta compilada del Frontend"
    }
    
    # Copiar Frontend
    Write-Log "Copiando archivos del Frontend desde: $script:FrontendDistPath" "INFO"
    
    try {
        Copy-Item -Path "$script:FrontendDistPath\*" -Destination $frontendDest -Recurse -Force -ErrorAction Stop
        
        $copiedFiles = Get-ChildItem -Path $frontendDest -Recurse -File
        $totalSize = ($copiedFiles | Measure-Object -Property Length -Sum).Sum / 1MB
        $sizeRounded = [math]::Round($totalSize, 2)
        Write-Log "Frontend copiado: $($copiedFiles.Count) archivos (${sizeRounded} MB)" "SUCCESS"
        
    } catch {
        Write-Log "Error al copiar Frontend: $_" "ERROR"
        throw
    }
    ...
    if (-not (Test-Path "$env:TEMP\api-publish\API.dll")) {
        throw "No se encontro API.dll en $env:TEMP\api-publish"
    }
    
    try {
        # Copiar con robocopy para mejor manejo de archivos bloqueados
        Write-Log "Usando robocopy para copiar archivos del API..." "INFO"
        
        $robocopyResult = robocopy "$env:TEMP\api-publish" $apiDest /E /R:3 /W:5 /NFL /NDL /NJH /NJS
        $robocopyExitCode = $LASTEXITCODE
        
        # Robocopy exit codes: 0-7 son exitosos
        if ($robocopyExitCode -le 7) {
            $apiFiles = Get-ChildItem -Path $apiDest -Recurse -File
            Write-Log "API copiado: $($apiFiles.Count) archivos" "SUCCESS"
        } else {
            # Si robocopy falla, intentar con Copy-Item
            Write-Log "Robocopy fallo, intentando con Copy-Item..." "WARNING"
            Copy-Item -Path "$env:TEMP\api-publish\*" -Destination $apiDest -Recurse -Force -ErrorAction Stop
        ...
        Write-Log "Error al copiar API: $_" "ERROR"
        Write-Log "Algunos archivos pueden estar en uso. Continuando..." "WARNING"
        
        # Listar archivos que no se copiaron
        $sourceFiles = Get-ChildItem "$env:TEMP\api-publish" -Recurse -File
        $destFiles = Get-ChildItem $apiDest -Recurse -File -ErrorAction SilentlyContinue
        
        if ($sourceFiles.Count -ne $destFiles.Count) {
            Write-Log "ADVERTENCIA: No todos los archivos se copiaron ($($destFiles.Count) de $($sourceFiles.Count))" "WARNING"
        }
    }
    
    # ============================================
    # CREAR WEB.CONFIG PARA API
    # ============================================
    ...
    <system.webServer>
      ...
      <aspNetCore processPath="dotnet" 
        ...
        <environmentVariables>
          ...
          <environmentVariable name="ASPNETCORE_HTTPS_PORT" value="$ApiPortHttps" />
        </environmentVariables>
      </aspNetCore>
    </system.webServer>
  </location>
...
"@
    ...
    try {
        ...
        Write-Log "web.config creado correctamente" "SUCCESS"
    } catch {
        Write-Log "Error al crear web.config: $_" "ERROR"
        throw
    }
    
    # Verificar que API.dll existe
    if (Test-Path "$apiDest\API.dll") {
        $apiDllSize = (Get-Item "$apiDest\API.dll").Length / 1MB
        $dllSizeRounded = [math]::Round($apiDllSize, 2)
        Write-Log "API.dll verificado (${dllSizeRounded} MB)" "SUCCESS"
    } else {
        throw "API.dll no se copio correctamente"
    }
...
function Invoke-DatabaseMigrations {
    Write-Log "Ejecutando migraciones de Entity Framework Core..." "INFO"
    ...
    try {
        ...
        # ============================================
        # MIGRACIÓN PARA AppDbContext (Base de datos principal)
        # ============================================
        Write-Log "Aplicando migraciones para AppDbContext en base de datos: $DatabaseName" "INFO"
        ...
        try {
            ...
            if (-not $efToolInstalled) {
                Write-Log "Instalando dotnet-ef tool..." "INFO"
                dotnet tool install --global dotnet-ef 2>&1 | Out-Null
            }
            
            # Aplicar migraciones para AppDbContext
            Write-Log "Ejecutando: dotnet ef database update --context AppDbContext" "INFO"
            
            $efOutput = dotnet ef database update --context AppDbContext --connection $connectionString --project API.csproj 2>&1
            
            if ($LASTEXITCODE -eq 0) {
                Write-Log "Migraciones de AppDbContext aplicadas correctamente" "SUCCESS"
            } else {
                Write-Log "Advertencia al aplicar migraciones de AppDbContext: $efOutput" "WARNING"
                Write-Log "La base de datos puede estar actualizada o hay un problema menor" "WARNING"
            }
            
        } catch {
            Write-Log "Error al aplicar migraciones de AppDbContext: $_" "WARNING"
            Write-Log "Continuando con el despliegue..." "WARNING"
        }
        
        # ============================================
        # MIGRACIÓN PARA CorporacionDbContext (Base de datos Clinica)
        # ============================================
        if ($ClinicaDatabaseName) {
            Write-Log "Aplicando migraciones para CorporacionDbContext en base de datos: $ClinicaDatabaseName" "INFO"
            
            $clinicaConnectionString = if ($SqlUser) {
                "Server=$SqlServer;Database=$ClinicaDatabaseName;User Id=$SqlUser;Password=$SqlPassword;TrustServerCertificate=True;"
            } else {
                "Server=$SqlServer;Database=$ClinicaDatabaseName;Integrated Security=True;TrustServerCertificate=True;"
            }
            
            try {
                Write-Log "Ejecutando: dotnet ef database update --context CorporacionDbContext" "INFO"
                
                $efClinicaOutput = dotnet ef database update --context CorporacionDbContext --connection $clinicaConnectionString --project API.csproj 2>&1
                
                if ($LASTEXITCODE -eq 0) {
                    Write-Log "Migraciones de CorporacionDbContext aplicadas correctamente" "SUCCESS"
                } else {
                    Write-Log "Advertencia al aplicar migraciones de CorporacionDbContext: $efClinicaOutput" "WARNING"
                    Write-Log "La base de datos puede estar actualizada o hay un problema menor" "WARNING"
                }
                
            } catch {
                Write-Log "Error al aplicar migraciones de CorporacionDbContext: $_" "WARNING"
                Write-Log "Continuando con el despliegue..." "WARNING"
            }
        }
        
        Write-Log "Proceso de migraciones completado" "SUCCESS"
        
    } catch {
        Write-Log "Error general durante las migraciones: $_" "ERROR"
        Write-Log "Las migraciones fallaron, pero el despliegue continuará" "WARNING"
        Write-Log "Puede ejecutar las migraciones manualmente con:" "WARNING"
        Write-Log "  dotnet ef database update --context AppDbContext --project API/API.csproj" "WARNING"
        Write-Log "  dotnet ef database update --context CorporacionDbContext --project API/API.csproj" "WARNING"
    } finally {
        Pop-Location
    }
}

function Invoke-DatabaseSeedData {
    Write-Log "Ejecutando seeding de datos iniciales..." "INFO"
    ...
    try {
        ...
        # ============================================
        # CREAR ARCHIVO TEMPORAL DE SEEDING
        # ============================================
        Write-Log "Creando herramienta de seeding temporal..." "INFO"
        
        $seedToolPath = Join-Path $env:TEMP "DbSeedTool"
        $seedProgramCs = Join-Path $seedToolPath "Program.cs"
        $seedCsproj = Join-Path $seedToolPath "DbSeedTool.csproj"
        
        # Crear directorio
        if (Test-Path $seedToolPath) {
            Remove-Item $seedToolPath -Recurse -Force
        }
        New-Item -ItemType Directory -Path $seedToolPath -Force | Out-Null
        
        # IMPORTANTE: Escapar backslashes para C#
        $apiPathEscaped = $apiPath -replace '\\', '\\'
        $sqlServerEscaped = $SqlServer -replace '\\', '\\'
        
        # Crear Program.cs con IConfiguration configurado correctamente
        $seedProgramContent = @"
...
var apiConnectionString = args.Length > 0 ? args[0] : throw new Exception("Connection string requerido");

// Configurar servicios
var services = new ServiceCollection();

// Configurar IConfiguration con appsettings.json
var configBuilder = new ConfigurationBuilder()
    .SetBasePath("$apiPathEscaped")
    .AddJsonFile("appsettings.json", optional: false, reloadOnChange: false)
    .AddInMemoryCollection(new Dictionary<string, string>
    {
        { "DatabaseSettings:ServerName", "$sqlServerEscaped" }
    });
...
// Configurar Identity
services.AddIdentity<AppUserIdentity, AppIdentityRole>(options =>
{
...
try
{
    ...
    // Ejecutar seeding
    await DbInitializer.SeedData(context, userManager, roleManager, config);
...
"@
        ...
        # Crear .csproj con versiones compatibles
        $seedCsprojContent = @"
<Project Sdk="Microsoft.NET.Sdk">
  ...
  <ItemGroup>
    <ProjectReference Include="$apiPathEscaped\API.csproj" />
  </ItemGroup>
  <ItemGroup>
...
"@
        ...
        # Guardar archivos
        $seedProgramContent | Out-File -FilePath $seedProgramCs -Encoding UTF8 -Force
        $seedCsprojContent | Out-File -FilePath $seedCsproj -Encoding UTF8 -Force
        
        Write-Log "Archivos de herramienta creados correctamente" "SUCCESS"
        Write-Log "Ruta API escapada: $apiPathEscaped" "INFO"
        
        # ============================================
        # EJECUTAR SEEDING PARA AppDbContext
        # ============================================
        Write-Log "Ejecutando seeding para AppDbContext en base de datos: $DatabaseName" "INFO"
        
        try {
            Push-Location $seedToolPath
            
            # Compilar y ejecutar
            Write-Log "Compilando herramienta de seeding..." "INFO"
            $buildOutput = dotnet build --configuration Release 2>&1
            
            if ($LASTEXITCODE -ne 0) {
                Write-Log "Salida de compilacion: $buildOutput" "ERROR"
                throw "Error compilando herramienta de seeding"
            }
            
            Write-Log "Ejecutando seeding de datos..." "INFO"
            $seedOutput = dotnet run --no-build --configuration Release -- "$connectionString" 2>&1
            
            if ($LASTEXITCODE -eq 0) {
                Write-Log "Seeding de AppDbContext completado correctamente" "SUCCESS"
                Write-Log "Salida: $seedOutput" "INFO"
            } else {
                Write-Log "Advertencia durante seeding: $seedOutput" "WARNING"
            }
            ...
            Pop-Location
            Write-Log "Error durante seeding de AppDbContext: $_" "WARNING"
            Write-Log "Los datos iniciales pueden no estar disponibles" "WARNING"
        }
        
        # ============================================
        # NOTA SOBRE CorporacionDbContext
        # ============================================
        Write-Log "NOTA: CorporacionDbContext se inicializa automaticamente al primer uso" "INFO"
        Write-Log "El seeding de CorporacionDbContext se ejecuta via CorporacionDbInitializer.SeedData" "INFO"
        
        # Limpiar archivos temporales
        if (Test-Path $seedToolPath) {
            Remove-Item $seedToolPath -Recurse -Force -ErrorAction SilentlyContinue
        }
        
        Write-Log "Proceso de seeding completado" "SUCCESS"
        
    } catch {
        Write-Log "Error general durante el seeding: $_" "ERROR"
        Write-Log "El seeding fallo, pero el despliegue continuará" "WARNING"
        Write-Log "Puede ejecutar el seeding manualmente iniciando la aplicacion" "WARNING"
    } finally {
        Pop-Location
    }
}
...
try {
    ...
    Invoke-DatabaseSeedData
    Configure-SqlPermissions
    Deploy-ToIIS
    New-IISSites
    Add-FirewallRules
    ...
    Write-Log "Configuracion Azure AD:" "INFO"
    Write-Log "  Client ID: Configurado desde .env" "INFO"
    Write-Log "  Redirect URI: https://${DomainName}/pages/users/user-info" "INFO"
    Write-Log "" "INFO"
    ...
    if ($UseSelfSignedCert) {
        ...
        Write-Log "Para Azure AD, registra estas URLs en el portal:" "WARNING"
        Write-Log "  - https://${DomainName}/pages/users/user-info" "WARNING"
        Write-Log "  - https://${DomainName}/" "WARNING"
    }
